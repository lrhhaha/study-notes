...
其次是如何添加宏任务和微任务，参数setTimeout和Promise的then函数，可以发现它们都是接受一个参数作为参数，然后在适当的时机再调用。\
...
3. 所谓的主任务是什么？是第一个script吗？其实不太清楚时间循环的起点应该如何描述
4. 说worker模拟JS主线程，这样可以吗？因为时间循环机制也是由主线程管理的吗