...
## 进一步封装
上面展示的代码已经完成了我们最初的需求，但大家应该也发现了，microQueue、macroQueue、pushMicro、pushMacro都是全局的，显得比较“散装”。

接下来我们将对worker改装成一个类，然后把上述四个变量和函数作为它的私有属性和方法，以达到封装的效果。

具体设计如下：
1. microQueue和microQueue是全局唯一，且不对外暴露的任务列表，所以将作为静态私有属性。
2. pushMacro和pushMicro其实就是普通的宏任务和微任务，所以把它们作为类的静态方法，可贡外界直接调用。
...
class Workers {
  ...
  run(callback) {
    function handleQueue() {
      while (Workers.#microQueue.length) {
        const fn = Workers.#microQueue.shift();
        fn();
      }

      if (Workers.#macroQueue.length) {
        const fn = Workers.#macroQueue.shift();
        fn();
      }

      if (Workers.#microQueue.length || Workers.#macroQueue.length) handleQueue();
    }
